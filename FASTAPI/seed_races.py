...
async def seed():
    async with async_session() as session:  # твой асинхронный сессион
        for race in race_data:
            existing = await session.execute(select(Race).where(Race.code == race["code"]))
